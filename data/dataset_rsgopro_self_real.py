...
class RSGOPRO(Dataset):
    ...
    def _generate_samples(self, path):
        ...
        for seq in seqs:
            ...
            for i in range(self.num_pf+st_num, seq_num - self.num_ff+st_num):
                sample = dict()
                sample['RS'] = {}
                if seq == 'r0' or seq == 'r1':
                    sample['RS']['t2b'] = [join(seq_path, '{:01d}_t2b.png'.format(j)) for j in
                                           range(i - self.num_pf, i + self.num_ff + 1)]  #:01d
                    sample['RS']['b2t'] = [join(seq_path, '{:01d}_b2t.png'.format(j)) for j in
                                           range(i - self.num_pf, i + self.num_ff + 1)]  #
                else:
                    sample['RS']['t2b'] = [join(seq_path, '{:08d}_rs_t2b.png'.format(j)) for j in
                                           range(i - self.num_pf, i + self.num_ff + 1)]  #:01d
                    sample['RS']['b2t'] = [join(seq_path, '{:08d}_rs_b2t.png'.format(j)) for j in
                                           range(i - self.num_pf, i + self.num_ff + 1)]  #:01d
