...
from models.AMT.blocks.raft import (
    coords_grid,
    SmallUpdateBlock, BidirCorrBlock
)
from models.AMT.blocks.feat_enc import (
    SmallEncoder
)
from models.AMT.blocks.ifrnet import (
    resize,
    Encoder,
    InitDecoder,
    IntermediateDecoder
)
from models.AMT.blocks.multi_flow import (
    multi_flow_combine,
    MultiFlowDecoder
)
...
class Model(nn.Module):
    ...
    def _corr_scale_lookup(self, coord, flow0, flow1, embt, downsample=1):
        ...
        embt = F.interpolate(embt, size=flow1.size()[-2:], mode='bilinear', align_corners=False,
                                    recompute_scale_factor=False)
        num = embt.shape[1] // 2
        # print(coord.shape, flow1.shape, embt.shape)
        corr0, corr1 = coord + flow1 * embt[:, :num], coord + flow0 * embt[:, num:]
        # corr_fn(coord + flow1 * embt[:, :num], coord + flow0 * embt[:, num:])
        corr = torch.cat([corr0, corr1], dim=1)
        flow = torch.cat([flow0, flow1], dim=1)
        return corr, flow

    def forward(self, x, time_stamps, scale_factor=1.0):
        img0, img1 = x[:, 0], x[:, 1]
        ...
        for t in range(len(time_stamps)):
            ######################################### the 4th decoder #########################################
            up_flow0_4, up_flow1_4, ft_3_ = self.decoder4(f0_4, f1_4, time_stamps[t])
            corr_4, flow_4 = self._corr_scale_lookup(coord,
                                                     up_flow0_4, up_flow1_4,
                                                     time_stamps[t], downsample=1)
            ...
            corr_3, flow_3 = self._corr_scale_lookup(coord, up_flow0_3, up_flow1_3,
                                                     time_stamps[t], downsample=2)
            ...
            corr_2, flow_2 = self._corr_scale_lookup(coord, up_flow0_2, up_flow1_2,
                                                     time_stamps[t], downsample=4)
            ...
            img_pred_list.append(imgt_pred)

        return img_pred_list
        # if eval:
