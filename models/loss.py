...
class Census(nn.Module):
    ...
    def forward(self, img0, img1):
        ...
        return (self.hamming(img0, img1) * self.valid_mask(img0, 3)).mean()
...
def upsample(x):
    ...
    cc = torch.cat([cc, torch.zeros(x.shape[0], x.shape[1], x.shape[3], x.shape[2] * 2).to(cc.device)], dim=3)
...
class LapLoss(torch.nn.Module):
    ...
    def forward(self, input, target):
        # self.gauss_kernel = self.gauss_kernel.to(input.device)
        pyr_input = laplacian_pyramid(img=input, kernel=self.gauss_kernel.to(input.device), max_levels=self.max_levels)
        pyr_target = laplacian_pyramid(img=target, kernel=self.gauss_kernel.to(input.device), max_levels=self.max_levels)
        return sum(torch.nn.functional.l1_loss(a, b) for a, b in zip(pyr_input, pyr_target)).mean()
