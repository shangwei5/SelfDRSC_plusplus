...
from models.DI_RIFE.model.RIFE_sdi_recur import Model
# from models.DI_EMA_VFI.benchmark.utils.padder import InputPadder
...
class ModelSRSCRSG(ModelPlain):
    ...
    def __init__(self, opt):
        ...
        self.net_rsg = Model() #net(load_pretrain=True, pretrain_fn=self.opt['path']['pretrained_rsg'], device='cuda')
        self.net_rsg.load_model(path=self.opt['path']['pretrained_rsg'])
        self.net_rsg.eval()

        # self.net_rsg.device()  #self.net_rsg = self.model_to_device(self.net_rsg)
    ...
    def netG_forward(self, is_train=True):
        ...
        if is_train:
            # b, n, c, h, w = self.time_rsc.shape  # (8, 2*ext_frames, 1, 256, 256)  #t2b:0,1,...   b2t:0,1,...
            time_rsc = []  #self.time_rsc.reshape(b, n * c, h, w)
            for ij in range(self.time_rsc.shape[1]//2):
                time_rsc.append(torch.cat((self.time_rsc[:,ij], self.time_rsc[:,ij+self.time_rsc.shape[1]//2]),dim=1))
            self.E = self.netG(self.L, time_rsc)   #b, num_frames*3, h, w   -----   b, num_frames*2*2, h, w
            self.E = torch.stack(self.E, dim=1)
            assert self.E.shape[1] == 3, self.E.shape
        else:
            # assert self.all_time_rsc.size()[1]//2 == 9, self.all_time_rsc.size()[1]
            time_rsc = []  # self.time_rsc.reshape(b, n * c, h, w)
            for ij in range(self.all_time_rsc.shape[1] // 2):
                time_rsc.append(
                    torch.cat((self.all_time_rsc[:, ij], self.all_time_rsc[:, ij + self.all_time_rsc.shape[1] // 2]), dim=1))
            self.E = self.netG(self.L, time_rsc)  # b, num_frames*3, h, w   -----   b, num_frames*2*2, h, w
            self.E = torch.stack(self.E, dim=1)
            # assert self.E.shape[1] == 9, self.E.shape
        ...
        if is_train:
            ...
            # mid = self.E[:, 0]
            # pre_mid = self.E[:, 0]
            # for j in range(1):  # iters=2
            #     pre_embt = (time_coding1 * float(j)) / float(1)
            #     cur_embt = (time_coding1 * (float(j) + 1)) / float(1)
            #     mid = self.net_rsg.inference(self.E[:, 0], self.E[:, 2], pre_mid, cur_embt, pre_embt)
            #     pre_mid = mid
            # L_t2b_0 = mid
            L_t2b_0 = self.net_rsg.inference(self.E[:, 0], self.E[:, 2], sdi_map=time_coding1, iters=1)

            # pre_mid = self.E[:, 2]
            # for j in range(1):  # iters=2
            #     pre_embt = ((1-time_coding1) * float(j)) / float(1)
            #     cur_embt = ((1-time_coding1) * (float(j) + 1)) / float(1)
            #     mid = self.net_rsg.inference(self.E[:, 2], self.E[:, 0], pre_mid, cur_embt, pre_embt)
            #     pre_mid = mid
            # L_t2b_2 = mid
            L_t2b_2 = self.net_rsg.inference(self.E[:, 2], self.E[:, 0], sdi_map=1-time_coding1, iters=1)
            self.L_t2b = (1 - time_coding1) * L_t2b_0 + time_coding1 * L_t2b_2

            # pre_mid = self.E[:, 0]
            # for j in range(1):  # iters=2
            #     pre_embt = (time_coding2 * float(j)) / float(1)
            #     cur_embt = (time_coding2 * (float(j) + 1)) / float(1)
            #     mid = self.net_rsg.inference(self.E[:, 0], self.E[:, 2], pre_mid, cur_embt, pre_embt)
            #     pre_mid = mid
            # L_b2t_0 = mid
            L_b2t_0 = self.net_rsg.inference(self.E[:, 0], self.E[:, 2], sdi_map=time_coding2, iters=1)

            # pre_mid = self.E[:, 2]
            # for j in range(1):  # iters=2
            #     pre_embt = ((1-time_coding2) * float(j)) / float(1)
            #     cur_embt = ((1-time_coding2) * (float(j) + 1)) / float(1)
            #     mid = self.net_rsg.inference(self.E[:, 2], self.E[:, 0], pre_mid, cur_embt, pre_embt)
            #     pre_mid = mid
            # L_b2t_2 = mid
            L_b2t_2 = self.net_rsg.inference(self.E[:, 2], self.E[:, 0], sdi_map=1-time_coding2, iters=1)
            self.L_b2t = (1 - time_coding2) * L_b2t_0 + time_coding2 * L_b2t_2
            ...
            # pre_mid = self.E[:, 0]
            # for j in range(1):  # iters=2
            #     pre_embt = (mid_time_coding1_up * float(j)) / float(1)
            #     cur_embt = (mid_time_coding1_up * (float(j) + 1)) / float(1)
            #     mid = self.net_rsg.inference(self.E[:, 0], self.E[:, 1], pre_mid, cur_embt, pre_embt)
            #     pre_mid = mid
            # warped_img_f_0 = mid
            warped_img_f_0 = self.net_rsg.inference(self.E[:, 0], self.E[:, 1], sdi_map=mid_time_coding1_up, iters=1)

            # pre_mid = self.E[:, 1]
            # for j in range(1):  # iters=2
            #     pre_embt = ((1-mid_time_coding1_up) * float(j)) / float(1)
            #     cur_embt = ((1-mid_time_coding1_up) * (float(j) + 1)) / float(1)
            #     mid = self.net_rsg.inference(self.E[:, 1], self.E[:, 0], pre_mid, cur_embt, pre_embt)
            #     pre_mid = mid
            # warped_img_f_1 = mid
            warped_img_f_1 = self.net_rsg.inference(self.E[:, 1], self.E[:, 0], sdi_map=1-mid_time_coding1_up, iters=1)
            warped_img_f = (1 - mid_time_coding1_up) * warped_img_f_0 + mid_time_coding1_up * warped_img_f_1
            ...
            # pre_mid = self.E[:, 1]
            # for j in range(1):  # iters=2
            #     pre_embt = (mid_time_coding1_down * float(j)) / float(1)
            #     cur_embt = (mid_time_coding1_down * (float(j) + 1)) / float(1)
            #     mid = self.net_rsg.inference(self.E[:, 1], self.E[:, 2], pre_mid, cur_embt, pre_embt)
            #     pre_mid = mid
            # warped_img_b_1 = mid
            warped_img_b_1 = self.net_rsg.inference(self.E[:, 1], self.E[:, 2], sdi_map=mid_time_coding1_down, iters=1)

            # pre_mid = self.E[:, 2]
            # for j in range(1):  # iters=2
            #     pre_embt = ((1 - mid_time_coding1_down) * float(j)) / float(1)
            #     cur_embt = ((1 - mid_time_coding1_down) * (float(j) + 1)) / float(1)
            #     mid = self.net_rsg.inference(self.E[:, 2], self.E[:, 1], pre_mid, cur_embt, pre_embt)
            #     pre_mid = mid
            # warped_img_b_2 = mid
            warped_img_b_2 = self.net_rsg.inference(self.E[:, 2], self.E[:, 1], sdi_map=1 - mid_time_coding1_down, iters=1)
            warped_img_b = (1 - mid_time_coding1_down) * warped_img_b_1 + mid_time_coding1_down * warped_img_b_2
            ...
            # pre_mid = self.E[:, 0]
            # for j in range(1):  # iters=2
            #     pre_embt = (mid_time_coding2_up * float(j)) / float(1)
            #     cur_embt = (mid_time_coding2_up * (float(j) + 1)) / float(1)
            #     mid = self.net_rsg.inference(self.E[:, 0], self.E[:, 1], pre_mid, cur_embt, pre_embt)
            #     pre_mid = mid
            # warped_img_f_0 = mid
            warped_img_f_0 = self.net_rsg.inference(self.E[:, 0], self.E[:, 1], sdi_map=mid_time_coding2_up, iters=1)

            # pre_mid = self.E[:, 1]
            # for j in range(1):  # iters=2
            #     pre_embt = ((1 - mid_time_coding2_up) * float(j)) / float(1)
            #     cur_embt = ((1 - mid_time_coding2_up) * (float(j) + 1)) / float(1)
            #     mid = self.net_rsg.inference(self.E[:, 1], self.E[:, 0], pre_mid, cur_embt, pre_embt)
            #     pre_mid = mid
            # warped_img_f_1 = mid
            warped_img_f_1 = self.net_rsg.inference(self.E[:, 1], self.E[:, 0], sdi_map=1 - mid_time_coding2_up, iters=1)
            warped_img_f = (1 - mid_time_coding2_up) * warped_img_f_0 + mid_time_coding2_up * warped_img_f_1
            ...
            # pre_mid = self.E[:, 1]
            # for j in range(1):  # iters=2
            #     pre_embt = (mid_time_coding2_down * float(j)) / float(1)
            #     cur_embt = (mid_time_coding2_down * (float(j) + 1)) / float(1)
            #     mid = self.net_rsg.inference(self.E[:, 1], self.E[:, 2], pre_mid, cur_embt, pre_embt)
            #     pre_mid = mid
            # warped_img_b_1 = mid
            warped_img_b_1 = self.net_rsg.inference(self.E[:, 1], self.E[:, 2], sdi_map=mid_time_coding2_down, iters=1)

            # pre_mid = self.E[:, 2]
            # for j in range(1):  # iters=2
            #     pre_embt = ((1 - mid_time_coding2_down) * float(j)) / float(1)
            #     cur_embt = ((1 - mid_time_coding2_down) * (float(j) + 1)) / float(1)
            #     mid = self.net_rsg.inference(self.E[:, 2], self.E[:, 1], pre_mid, cur_embt, pre_embt)
            #     pre_mid = mid
            # warped_img_b_2 = mid
            warped_img_b_2 = self.net_rsg.inference(self.E[:, 2], self.E[:, 1], sdi_map=1 - mid_time_coding2_down, iters=1)
            warped_img_b = (1 - mid_time_coding2_down) * warped_img_b_1 + mid_time_coding2_down * warped_img_b_2
            warped_img_b = warped_img_b * (1 - mid_mask2)
            self.L_b2t_mid = warped_img_b + warped_img_f


            save = torch.cat((self.L[:, 0], self.L_t2b, self.L_t2b_mid), dim=-1).clamp(0, 1)[0].permute(1, 2, 0).detach().cpu().numpy()
            save = (save * 255.).astype(np.uint8)
            cv2.imwrite('./t2b_train_rife_dr.png', save)
            save = torch.cat((self.L[:, 1], self.L_b2t, self.L_b2t_mid), dim=-1).clamp(0, 1)[0].permute(1, 2,
                                                                                                        0).detach().cpu().numpy()
            save = (save * 255.).astype(np.uint8)
            cv2.imwrite('./b2t_train_rife_dr.png', save)
